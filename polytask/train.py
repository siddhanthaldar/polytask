...
class Workspace:
	def __init__(self, cfg):
		...
		try:
			# for distral
			self.cfg.agent.num_envs = len(self.train_env)
		except:
			pass
	...
	def setup(self):
		...
		obs_shape = self.train_env[0].observation_spec()[self.cfg.obs_type].shape
		goal_dtype = np.uint8 if self.cfg.suite.name in ['metaworld', 'kitchen', 'robotgym'] else np.float32
		data_specs = [
	...
	def replay_iter(self):
		for env_idx in range(len(self._replay_iter)):
			if self._replay_iter[env_idx] is None:
				self._replay_iter[env_idx] = iter(self.replay_loader[env_idx])
	...
	def train(self):
		...
		env_idx = random.randint(0, len(self.train_env)-1)
		...
		if repr(self.agent) not in ['bc']:
			# get goal
			if self.cfg.suite.name in ['metaworld', 'kitchen', 'robotgym']:
				if self.cfg.multitask:
					goal_idx = np.random.randint(env_idx*self.cfg.num_demos_per_task, (env_idx+1)*self.cfg.num_demos_per_task)
					goal = self.expert_goal[goal_idx]
				else:
		...
		while train_until_step(self.global_step):
			if repr(self.agent) not in ['bc']:
				if time_step.last():
					...
					for i, elt in enumerate(time_steps):
						...
						self.replay_storage[env_idx].add(elt)
					...
					if self.cfg.suite.save_snapshot:
						self.save_snapshot(f"env_{env_idx}")
			...
			if not seed_until_step(self.global_step):
				...
				metrics = self.agent.update(self.replay_iter, self.expert_replay_iter, self.global_step, 
											self.cfg.bc_regularize, goal_embedding_func, task_id)

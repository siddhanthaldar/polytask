...
import utils
from agent.networks.encoder import ProgressiveEncoder as Encoder
from agent.networks.mlp import MLP
from agent.networks.progressive_net import PNN
...
class Actor(nn.Module):
	def __init__(self, repr_dim, action_shape, hidden_dim):
		...
		self.policy = PNN(len(self.layers)-1, layer_type='linear', no_relu=True)
	...
	def new_task(self):
		...
		# add new column
		self.policy.new_task(self.layers)
	
	def parameters(self):
		return self.policy.parameters(-1)

	def forward(self, obs, goal, std, task_id=-1):

		obs = torch.cat([goal, obs], dim=-1)
		feat = self.policy(obs, task_id=task_id)
...
class Critic(nn.Module):
	def __init__(self, repr_dim, action_shape, hidden_dim):
		...
		self.layers = [
			repr_dim + goal_dim + self._repeat * action_shape[0],
			hidden_dim,
			hidden_dim,
			1
		]
		self.Q1 = PNN(len(self.layers)-1, layer_type='linear', no_relu=True)
		self.Q2 = PNN(len(self.layers)-1, layer_type='linear', no_relu=True)
	...
	def new_task(self):
		...
		# add new column
		self.Q1.new_task(self.layers)
		self.Q2.new_task(self.layers)
	
	def parameters(self):
		return list(self.Q1.parameters(-1)) + list(self.Q2.parameters(-1))
...
class DrQv2Agent:
	...
	def switch_task(self):
		...
		if self.use_encoder and self.train_encoder:
			...
			self.encoder_target = self.encoder_target.to(self.device)
	...
	def act(self, obs, goal, step, eval_mode, task_id=-1):
		obs = torch.as_tensor(obs, device=self.device).float()
		goal = torch.as_tensor(goal, device=self.device).float()

		obs = self.encoder(obs.unsqueeze(0), task_id=task_id) if self.use_encoder else obs.unsqueeze(0)
		goal = self.encoder(goal.unsqueeze(0), task_id=task_id) if self.use_encoder and len(goal.shape) == 3 else goal.unsqueeze(0)
		stddev = utils.schedule(self.stddev_schedule, step)

		dist = self.actor(obs, goal, stddev, task_id=task_id)

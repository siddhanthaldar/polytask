...
def get_model_and_assets(num_poles=1, xml_file_id=None, object_varied=None):
    """Returns a tuple containing the model XML string and a dict of assets."""
    return _make_model(num_poles, xml_file_id, object_varied), common.ASSETS
...
def balance(
    ...
    physics = Physics.from_xml_string(*get_model_and_assets(xml_file_id=xml_file_id, object_varied=object_varied))
...
def _make_model(n_poles, xml_file_id=None, object_varied=None):
    """Generates an xml string defining a cart with `n_poles` bodies."""
    if xml_file_id is not None and object_varied is not None:
        filename = f"cartpole_{object_varied}_mass_{xml_file_id}.xml"
        print(filename)
    else:

...
def make(name, height, width, frame_stack, action_repeat, seed, enable_arm, enable_gripper, enable_camera, x_limit, y_limit, z_limit, pitch, roll, keep_gripper_closed, yaw=0):
	if type(name) == str:
		env = gym.make(name, height=height, width=width, enable_arm=enable_arm, enable_gripper=enable_gripper, enable_camera=enable_camera, 
					x_limit=x_limit, y_limit=y_limit, z_limit=z_limit, pitch=pitch, roll=roll, yaw=yaw, keep_gripper_closed=keep_gripper_closed)
		env.seed(seed)
		...
		return env
	elif type(name) == omegaconf.listconfig.ListConfig:
		envs = []
		for i, env_name in enumerate(name):
			env = gym.make(env_name, height=height, width=width, enable_arm=enable_arm, enable_gripper=enable_gripper, enable_camera=enable_camera, 
					x_limit=x_limit, y_limit=y_limit, z_limit=z_limit, pitch=pitch, roll=roll, yaw=yaw, keep_gripper_closed=keep_gripper_closed)
			env.seed(seed)
			env = RGBArrayAsObservationWrapper(env, width=width, height=height)
			env = ActionDTypeWrapper(env, np.float32)

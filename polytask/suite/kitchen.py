...
class RGBArrayAsObservationWrapper(dm_env.Environment):
	...
	"""
	def __init__(self, env, width=224, height=224, max_path_length=280):
		self._env = env
		self._width = width
		self._height = height
	...
	def step(self, action):
		...
		obs['pixels'] = self.get_frame()
		obs['completions'] = info['completions']
		self.episode_step += 1
	...
	def render(self, mode="rgb_array", width=256, height=256):
		camera = engine.MovableCamera(self._env.sim, width=width, height=height)
		camera.set_pose(
	...
	def get_frame(self):
		return self.render(mode="rgb_array", width=self._width, height=self._height)
...
class FrameStackWrapper(dm_env.Environment):
	...
	def _transform_observation(self, time_step):
		...
		obs['completions'] = time_step.observation['completions']
...
def make(name, frame_stack, action_repeat, seed, task):
	if type(task) == str:
		env = gym.make(name)
		env.seed(seed)

		# set task
		env.env.tasks_to_complete = set([TASK_ELEMENTS[task]])

		MAX_PATH_LENGTH = 280

		# add wrappers
		env = RGBArrayAsObservationWrapper(env, max_path_length=MAX_PATH_LENGTH)
		env = ActionDTypeWrapper(env, np.float32)
		env = ActionRepeatWrapper(env, action_repeat)
		env = FrameStackWrapper(env, frame_stack)
		env = ExtendedTimeStepWrapper(env)
		return env
	elif type(task) == omegaconf.listconfig.ListConfig:
		envs = []
		for t in task:
			env = gym.make(name)
			env.seed(seed)
			...
			# set task
			env.env.tasks_to_complete = set([TASK_ELEMENTS[t]])

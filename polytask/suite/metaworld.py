...
from .metaworld_info import CAMERA, MAX_PATH_LENGTH
...
def make(name, frame_stack, action_repeat, seed):
	if type(name) == str:
		...
		envs = []
		for n in name:
			ml1 = metaworld.ML1(n) # Construct the benchmark, sampling tasks
			env = ml1.train_classes[n]()  # Create an environment with task
			env.seed(seed)
			...
			# add wrappers
			env = RGBArrayAsObservationWrapper(env, ml1, max_path_length=MAX_PATH_LENGTH[n], camera_name=CAMERA[n])
			env = ActionDTypeWrapper(env, np.float32)
			env = ActionRepeatWrapper(env, action_repeat)
			env = FrameStackWrapper(env, frame_stack)
			env = ExtendedTimeStepWrapper(env)

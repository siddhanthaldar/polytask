...
class ExpertReplayBuffer(IterableDataset):
	def __init__(self, dataset_path, num_demos, obs_type):
		...
		# Store goals for goal conditioning
		self.goals = []
		for i in range(len(obses)):
			self.goals.append(obses[i][-1])
		
		self._episodes = []
		# for i in range(num_demos):
		i = 0
		while len(self._episodes) < num_demos:
			if i >= len(obses):
				break
			if len(obses[i]) < 20: # Only store episodes with at least 20 transitions
				i += 1
				continue
	...
	def _sample(self):
		...
		goal_idx = np.random.randint(0, len(self.goals))
...
class RelabeledReplayBuffer(IterableDataset):
...
def make_expert_replay_loader(replay_dir, batch_size, num_demos, obs_type, relabeled=False):
	if relabeled:
		iterable = RelabeledReplayBuffer(replay_dir)
	else:

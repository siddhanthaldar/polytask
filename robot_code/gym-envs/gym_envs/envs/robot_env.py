...
class RobotEnv(gym.Env):
	def __init__(self, home_displacement=[2,-0.21,.4], height=84, width=84, step_size=10, enable_arm=True, enable_gripper=True, enable_camera=True,
				 ...
				 x_limit=None, y_limit=None, z_limit=None, pitch=0, roll=180, yaw=0, goto_zero_at_init=False, start_at_the_back=False):
	...
	def init_arm(self):
		self.arm = XArm(home_displacement=self.home_displacement, keep_gripper_closed=self.keep_gripper_closed, highest_start=self.highest_start, 
						x_limit=self.x_limit, y_limit=self.y_limit, z_limit=self.z_limit, pitch=self.pitch, roll=self.roll, yaw=self.yaw, start_at_the_back=self.start_at_the_back)

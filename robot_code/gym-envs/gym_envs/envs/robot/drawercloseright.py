...
class RobotDrawerCloseRightEnv(robot_env.RobotEnv):
	def __init__(self, height=84, width=84, step_size=10, enable_arm=True, enable_gripper=True, enable_camera=True, camera_view='side',
				 use_depth=False, dist_threshold=0.05, random_start=True, x_limit=None, y_limit=None, z_limit=None, pitch=0, roll=180, yaw=0, keep_gripper_closed=True):
		robot_env.RobotEnv.__init__(
			self,
			home_displacement=[1.75, -1.6, 1.5],
			height=height,
			width=width,
			step_size=step_size,
	...
	def reset(self):
		...
		if self.random_start:
			self.arm.set_random_pos(only_x=True, y_left=True)
		# time.sleep(0.4)
		time.sleep(2)
		obs = {}

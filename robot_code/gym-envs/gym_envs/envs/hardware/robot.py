...
class XArm:
	...
	def __init__(self, config_file='./robot.conf', home_displacement = (0,0,0), low_range=(1,1,0.2) , high_range=(2,2,1),
				 keep_gripper_closed=False, highest_start=False, x_limit=None, y_limit=None, z_limit=None, pitch = 0, roll=180,
				 yaw=0, gripper_action_scale=200, start_at_the_back=False):
		self.arm = None
	...
	def reset(self, home = False, reset_at_home=False):
	...
	def get_random_pos(self, pos=None):
		...
		pos[2] = self.home[2] + z_disp if not self.highest_start else self.z_limit[1]

		if self.start_at_the_back:
			pos[0] = self.x_limit[0]
		return pos

	def set_random_pos(self, only_x=False, y_left=False):
		self.clear_errors()
		self.set_mode_and_state()
		pos = self.get_position()
		...
		if only_x:
			...
			pos[1] = self.y_limit[1] if not y_left else self.y_limit[0]
			pos[2] = self.z_limit[1]
		# Set random pos
		self.set_position(pos)
		if self.keep_gripper_closed:
	...
	def set_position(self, pos, wait=False):
		...
		self.arm.set_position(x=x, y=y, z=z, roll=self.roll, pitch=self.pitch, yaw=self.yaw, wait=wait)
